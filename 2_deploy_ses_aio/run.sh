...
pushd ${CURRENT_FOLDER} > /dev/null
    ...
    ansible-playbook ${VERBOSITY} prepare-ses-ansible.yml \
		 -i ${MAIN_FOLDER}/inventory-ses.ini
    pushd ${MAIN_FOLDER}/ses-ansible/
        ansible-playbook ${VERBOSITY} ses-install.yml \
		     -i inventory.ini -e ses_openstack_config=True
    popd
    ansible-playbook ${VERBOSITY} get-ses-data.yml \
		     -i ${MAIN_FOLDER}/inventory-ses.ini
    ansible-playbook ${VERBOSITY} set-user-variables.yml \
		     -i ${MAIN_FOLDER}/inventory-ses.ini
popd
