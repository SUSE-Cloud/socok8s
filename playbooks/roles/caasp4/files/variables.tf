...
# Commented above as download.suse.de cannot be resolved.
# https://cloudinit.readthedocs.io/en/latest/topics/examples.html?highlight=resolver#configure-an-instances-resolv-conf
# Also entries in variables file (variables.tfvars) is added to default so need to make it empty here.
variable "lb_repositories" {
  type        = "map"
  default     = {}
  description = "Urls of the repositories to mount for lb via cloud-init"
}
...
variable "sles_registry_code" {
  default     = ""
  description = "SUSE Linux Enterprise Server Product Registration Code"
}


variable "cidrhost_master_offset" {
  default     = 64
  description = "hostnum offset for master"
}

variable "cidrhost_lb_offset" {
  default     = 64
  description = "hostnum offset for load balancer"
}

variable "cidrhost_worker_offset" {
  default     = 128
  description = "hostnum offset for workers"
}

variable "network_bridge_name" {
  type        = "string"
  default     = "br1"
  description = "Network bridge name used by the cluster"
}
...
variable "domain_create_timeout" {
  type        = "string"
  default     = "5m"
  description = "Domain create timeout while waiting for domain to get IP lease."
}
