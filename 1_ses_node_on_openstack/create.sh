...
pushd ${MAIN_FOLDER}
    ...
    grep ${IP_CREATE} inventory-ses.ini >/dev/null 2>&1 || (echo "[ses_nodes]" > inventory-ses.ini && echo "${SERVER_NAME} ansible_host=${IP_CREATE} ansible_user=root" >> inventory-ses.ini)
